...
interface UnitTestConfig {
  ...
  focus: {
    critical: ['auth/*', 'reports/*', 'maps/*'];
    shared: ['@kentnabiz/shared/*', '@kentnabiz/ui/*'];
  };
}
```
...
interface IntegrationTestConfig {
  api: {
    framework: 'supertest';
    focus: ['auth-flow', 'report-flow', 'media-upload'];
  };

  ui: {
    framework: 'cypress';
    critical: ['login-flow', 'report-creation', 'map-interaction'];
  };
}
```
...
interface MVPTestScope {
  critical: {
    ...
    reports: ['creation', 'media-upload', 'filtering'];
    shared: ['api-client', 'form-validation', 'error-handling'];
  };
}
```
...
interface DeferredTests {
  advanced: ['stress-testing', 'visual-regression', 'accessibility'];

  e2e: ['complex-scenarios', 'edge-cases', 'full-mobile-tests'];
}
```
...
interface DevPipeline {
  triggers: ['pre-commit', 'pre-push', 'manual'];

  stages: {
    validate: ['lint', 'type-check', 'unit-tests'];
    test: ['critical-tests', 'integration-tests'];
  };
}
```
