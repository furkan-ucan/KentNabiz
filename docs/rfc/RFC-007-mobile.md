...
interface SharedUsage {
  components: {
    ...
    forms: ['ReportForm', 'LoginForm'];
  };

  logic: {
    hooks: ['useAuth', 'useReport', 'useLocation'];
    utils: ['validation', 'api-client', 'storage'];
  };
}
```
...
interface NativeModules {
  ...
  camera: {
    ...
    features: ['photo', 'flash', 'autofocus'];
  };
...
interface PlatformFeatures {
  ...
  ios: {
    minVersion: '13.0';
    permissions: ['NSCameraUsageDescription', 'NSLocationWhenInUseUsageDescription'];
  };
}
```
...
interface NavigationStructure {
  ...
  tabs: {
    items: ['HomeTab', 'ReportsTab', 'ProfileTab'];
    initialTab: 'HomeTab';
  };
}
```
...
interface ReduxIntegration {
  ...
  slices: {
    shared: ['auth', 'reports', 'notifications'];
    mobile: ['camera', 'location', 'offline'];
  };
}
```
...
interface MobileTests {
  scope: ['navigation', 'state-management', 'offline-logic'];
...
interface ManualTests {
  ...
  scenarios: ['offline-usage', 'background-sync', 'camera-usage'];
}
```
